...
export async function range(env,days,start,end){
 let endDate=end?new Date(end+'T00:00:00Z'):new Date(),startDate=start?new Date(start+'T00:00:00Z'):new Date(endDate);
 ...
 let trackingStart='';if(env.AKHISAVE_SETTINGS)trackingStart=await env.AKHISAVE_SETTINGS.get(trackingStartKey)||'';
 for(let i=0;i<max;i++){
 ...
 const rangeStart=startDate.toISOString().slice(0,10),rangeEnd=endDate.toISOString().slice(0,10),ready=!!trackingStart&&trackingStart<=rangeStart;
 const summary=ready?{visitors:vis.size,newVisitors:newVis.size,returning:Math.max(0,vis.size-newVis.size),pageViews,attempts,downloads,failures}:{visitors:null,newVisitors:null,returning:null,pageViews,attempts,downloads,failures};
 return{success:true,today:rows[rows.length-1]||{date:'',visitors:0,newVisitors:0,returning:0,pageViews:0,attempts:0,downloads:0,failures:0},days:rows,summary,tracking:{ready,start:trackingStart||null},topTools:tools};
}

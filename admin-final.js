...
export default {async fetch(request,env,ctx){
  ...
  html=html.replace('</body>',TAP_FIX+'<script src="/admin-login-fix.js?v=4"></script><script src="/admin-ui-fix.js?v=1"></script><script src="/admin-categories-fix.js?v=2"></script><script src="/admin-dashboard-fix.js?v=1"></script></body>');
  const headers=new Headers(response.headers);headers.delete('content-length');headers.set('Cache-Control','no-store');return new Response(html,{status:response.status,headers:h});
}};

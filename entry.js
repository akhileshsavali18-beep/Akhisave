...
const DEFAULT_EXTENDED={ads:{enabled:false,adsterra:false,adsterraId:"",monetag:false,zone:""},seo:{title:"Instagram Downloader - Photos, Reels, Videos & Stories | AkhiSave",description:"AkhiSave is a fast Instagram downloader to download public Instagram photos, reels, videos and stories by URL. No password required.",keywords:"",ogTitle:"",ogDescription:"",ogImage:""}};
async function extendedSettings(env){try{const raw=await env.AKHISAVE_SETTINGS.get('site_settings_extended');if(!raw)return structuredClone(DEFAULT_EXTENDED);const x=JSON.parse(raw)||{},a=x.ads&&typeof x.ads==='object'?x.ads:{},s=x.seo&&typeof x.seo==='object'?x.seo:{};return{ads:{enabled:Boolean(a.enabled),adsterra:Boolean(a.adsterra),adsterraId:cleanText(a.adsterraId,80),monetag:Boolean(a.monetag),zone:cleanText(a.zone,80)},seo:{title:cleanText(s.title||DEFAULT_EXTENDED.seo.title,140),description:cleanText(s.description||DEFAULT_EXTENDED.seo.description,220),keywords:cleanText(s.keywords,300),ogTitle:cleanText(s.ogTitle||s.title||DEFAULT_EXTENDED.seo.title,140),ogDescription:cleanText(s.ogDescription||s.description||DEFAULT_EXTENDED.seo.description,220),ogImage:cleanText(s.ogImage,500)}}}catch{return structuredClone(DEFAULT_EXTENDED)}}
...
export default {async fetch(request,env,ctx){
  ...
  if(url.pathname==='/api/admin/settings'){
    ...
    if(request.method==='PUT'){
      ...
      const next={ads:{enabled:Boolean(a.enabled),adsterra:Boolean(a.adsterra),adsterraId:cleanText(a.adsterraId,80),monetag:Boolean(a.monetag),zone:cleanText(a.zone,80)},seo:{title:cleanText(s.title||ext.seo.title,140),description:cleanText(s.description||ext.seo.description,220),keywords:cleanText(s.keywords,300),ogTitle:cleanText(s.ogTitle||s.title||ext.seo.title,140),ogDescription:cleanText(s.ogDescription||s.description||ext.seo.description,220),ogImage:cleanText(s.ogImage,500)}};
      await env.AKHISAVE_SETTINGS.put('site_settings_extended',JSON.stringify(next));
      const forwarded={...body,ads:{monetag:Boolean(next.ads.monetag),zone:next.ads.zone},seo:{title:next.seo.title,description:next.seo.description}};

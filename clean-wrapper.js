...
function cleanHtml(html){
  ...
  // ZERO ADS for now. Remove every known ad-network script, iframe, container and legacy ad slot.
  ...
  // Never allow the old combined uploaded logo to remain in rendered HTML.
  out=out.replaceAll(OLD_LOGO,COMBINED_LOGO);
  out=out.replaceAll("/akhisave-mark.svg",LOGO_ONLY);

  // Replace common legacy branding paths if they occur directly in HTML.
  out=out.replaceAll("/307a3722-6c83-4b6b-a3fa-a5a840bf5d4b.png",LOGO_ONLY);
  out=out.replaceAll("/4dc6e410-9139-4401-a2f8-84e67a0a29b2.png",COMBINED_LOGO);
  out=out.replaceAll("/eb358ee7-8d58-460f-87fa-feb2edd6cd3d.png",WORDMARK);

  // Correct logo roles by location. Header/brand = combined; login mark = logo-only; footer = name-only.
...
.akhisave-ad-banner,.akhisave-ad-slot,.ad,.ad-banner,.ad-slot,.social-bar,[id*="ad-"]{display:none!important}
  ...
  // Runtime safety guard: if an old ad script is somehow still injected after load, remove it.

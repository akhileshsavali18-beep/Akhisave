...
// Current uploaded branding assets: logo-only, combined logo+name, and name-only.
// The repo assets are mapped to their actual visual roles.
...
function cleanHtml(html){
  ...
  // Remove ad scripts by checking the complete <script>...</script> block.
  out=out.replace(/<script\\b[^>]*>[\\s\\S]*?<\\/script>/gi,(block)=>BAD_AD.test(block)?"":block);
  // Remove ad iframes even when the ad provider is present in an attribute.
  out=out.replace(/<iframe\\b[^>]*>[\\s\\S]*?<\\/iframe>/gi,(block)=>BAD_AD.test(block)?"":block);

  // Remove known ad containers and every ad placeholder generated by the old worker.
  ...
  // Replace the obsolete uploaded logo reference everywhere.
  out=out.replaceAll(OLD_LOGO,COMBINED_LOGO);
  out=out.replaceAll("/akhisave-mark.svg",LOGO_ONLY);

  // Website top bar: ONE combined logo+name image, exactly as requested.
...
@media(max-width:650px){.brand .brand-combined{width:150px!important;height:52px!important}.navlinks{gap:2px!important}.navlinks a{font-size:10px!important;padding:8px 7px!important}}
</style>`;

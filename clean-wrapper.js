...
const LEGAL_PAGES=new Set(["/privacy","/privacy.html","/terms","/terms.html","/dmca","/dmca.html","/contact","/contact.html","/faq","/faq.html"]);
...
function cleanHtml(html,path=""){
  ...
  // Keep the exact latest separate logo + name treatment on the website top bar.
  ...
  // Only these five secondary pages get the same latest logo treatment.
  if(LEGAL_PAGES.has(path)){
    out=out.replace(/(<header[^>]*>\s*(?:<div[^>]*class=["'][^"']*(?:nav|headin)[^"']*["'][^>]*>\s*)?)<a\s+class=["']brand["'][^>]*>[\s\S]*?<\/a>(\s*(?:<a\s+class=["']home["'][^>]*>[\s\S]*?<\/a>)?)/i,
      '$1<a class="brand" href="/" aria-label="AkhiSave home"><img class="brand-logo-only" src="'+LOGO_ONLY+'" alt="AkhiSave logo"><img class="brand-wordmark" src="'+WORDMARK+'" alt="AkhiSave"></a>$2');
    out=out.replace(/(<header[^>]*>\s*(?:<div[^>]*class=["'][^"']*(?:nav|headin)[^"']*["'][^>]*>\s*)?)<a\s+href=["']https:\/\/akhisave\.online\/["'][^>]*>[\s\S]*?<\/a>(\s*<\/header>)/i,
      '$1<a class="brand" href="/" aria-label="AkhiSave home"><img class="brand-logo-only" src="'+LOGO_ONLY+'" alt="AkhiSave logo"><img class="brand-wordmark" src="'+WORDMARK+'" alt="AkhiSave"></a>$2');
  }
...
async function cleanResponse(response,path=""){
...
export default{async fetch(request,env,ctx){const path=new URL(request.url).pathname;return cleanResponse(await app.fetch(request,env,ctx),path)}};
